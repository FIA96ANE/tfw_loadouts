...
if (isClass (configFile >> "CfgPatches" >> "task_force_radio")) then {
	...
	if (_givePersonalRadio) then {
		...
		_unit linkItem BLU_F_personal_tf_faction_radio;
		
		if (_givePersonalRadioBW) then {
			_unit linkItem BW_personal_tf_faction_radio;
		};
	}
	else {
		_unit linkItem BLU_F_rifleman_tf_faction_radio;
	};
} else {
